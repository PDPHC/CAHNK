ALTER TABLE public.module_data DROP CONSTRAINT module_data_module_name_check;
ALTER TABLE public.module_data ADD CONSTRAINT module_data_module_name_check CHECK(module_name IN ('homeroom','attendance','savings','behavior','health','scholarship','volunteer','literacy','homeroom_times','report_checks'));
...
CREATE OR REPLACE FUNCTION private.require_complete_teacher_submission()
RETURNS trigger LANGUAGE plpgsql SECURITY INVOKER SET search_path = '' AS $$
DECLARE errors jsonb;
BEGIN
 IF NEW.status='submitted_to_academic' AND EXISTS(SELECT 1 FROM public.profiles WHERE id=auth.uid() AND role='teacher') THEN
  errors:=public.validate_monthly_submission(NEW.classroom_id,NEW.report_month);
  IF jsonb_array_length(errors)>0 THEN RAISE EXCEPTION USING MESSAGE='ข้อมูลธุรการยังไม่ครบ ไม่สามารถส่งได้',DETAIL=errors::text; END IF;
 END IF;
 RETURN NEW;
END;
$$;
REVOKE ALL ON FUNCTION private.require_complete_teacher_submission() FROM PUBLIC;
CREATE TRIGGER trg_require_complete_teacher_submission BEFORE INSERT OR UPDATE ON public.classroom_submissions FOR EACH ROW EXECUTE FUNCTION private.require_complete_teacher_submission();
