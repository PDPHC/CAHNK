(() => {
 ...
 function pdf(report=null){
  ...
  const listener=e=>{if(e.origin!==location.origin||e.source!==child||e.data?.type!=='pp5-print-ready'||e.data.job!==job)return;child.postMessage({type:'pp5-print-data',job,patches:printPatches,report,title:'ปพ5_'+String(value('IN','Q7')||'รายวิชา')},location.origin);window.removeEventListener('message',listener);clearTimeout(timeout)};
 ...
 function render(){const panel=$('pp5Panel');panel.classList.add('pp5-form-panel');panel.dataset.formPage=tab;$('editCourse').hidden=!['scores','info'].includes(tab);if(tab==='info'){
  ...
  const groups=[['ก่อนกลางภาค',['H','I','J','K','L','M','N','O','P','Q']],['กลางภาค',['Z','AB','AD','AF','AH','AJ','AL','AN','AP','AR']],['แก้ / ซ่อม',['BB']],['หลังกลางภาค',['BD','BE','BF','BG','BH','BI','BJ','BK','BL','BM']],['ชิ้นงาน',['BW','BX']],['ปลายภาค',['CD','CE','CF','CG','CH','CI','CJ','CK','CL','CM']],['อ่าน คิดวิเคราะห์ เขียน',['CV','CW','CX','CY']],['คุณลักษณะอันพึงประสงค์',['DC','DD','DE','DF','DG','DH','DI','DJ']]];
  panel.innerHTML='<h2>กรอกคะแนนย่อยรายจุดประสงค์</h2><p class="pp5-note">กรอกเพื่อใช้คะแนนจริงแทนสูตรแจงคะแนนอัตโนมัติของช่องนั้น • เว้นว่างเพื่อกลับไปใช้สูตรเดิม • คะแนนรวมในขั้นตอน 2 ต้องตรวจให้สอดคล้องกับคะแนนย่อย</p>'+groups.map(([label,cols])=>'<details class="pp5-score-group" open><summary>'+label+'</summary><div class="table-wrap"><table class="pp5-table"><thead><tr><th>เลขที่</th><th>ชื่อ–สกุล</th>'+cols.map(c=>'<th>ข้อ '+esc(String(value('F',c+'5')).trim()||c)+'<br>เต็ม '+esc(String(value('F',c+'6')).trim()||'—')+'</th>').join('')+'</tr></thead><tbody>'+Array.from({length:60},(_,i)=>{if(!value('IN','C'+(i+4)))return '';return '<tr><td>'+(i+1)+'</td><td>'+esc(value('IN','D'+(i+4)))+'</td>'+cols.map(c=>'<td><input type="number" min="0" step="any" data-sheet="F" data-ref="'+c+(i+7)+'" aria-label="'+label+' ข้อ '+c+' เลขที่ '+(i+1)+'" placeholder="อัตโนมัติ" value="'+esc(patches['F!'+c+(i+7)]??'')+'" '+(!editable()?'disabled':'')+'></td>').join('')+'</tr>'}).join('')+'</tbody></table></div></details>').join('');
 }else if(tab==='schedule'){
  panel.innerHTML='<h2>ตั้งค่ารายสัปดาห์</h2><p class="pp5-note">กรอกวัน เดือน ปี เฉพาะสัปดาห์แรก แต่ละช่องแทนคาบเรียนตามต้นฉบับ สัปดาห์ถัดไปเพิ่มให้อีก 7 วัน</p><div class="table-wrap"><table class="pp5-table"><thead><tr><th>สัปดาห์</th>'+[1,2,3,4,5].map(n=>'<th>คาบที่ '+n+'</th>').join('')+'</tr></thead><tbody><tr><td>1</td>'+['F','G','H','I','J'].map(c=>'<td>'+input('W1',c+'5',{numeric:true,label:'วันที่เรียนคาบ '+c})+'</td>').join('')+'</tr>'+Array.from({length:19},(_,i)=>'<tr><td>'+(i+2)+'</td>'+['F','G','H','I','J'].map(c=>'<td data-week-date="'+c+'" data-week-offset="'+(i+1)+'">'+(value('W1',c+'5')?serialToDate(Number(value('W1',c+'5'))+(i+1)*7):'—')+'</td>').join('')+'</tr>').join('')+'</tbody></table></div>';
 }else if(tab==='attendance'){
  panel.innerHTML='<h2>เช็กการมาเรียน</h2><p class="pp5-note">/ มาเรียน • - ไม่มีคาบเรียน • ป ป่วย • ล ลา • ส สาย • ข ขาด • ตั้งวันที่เรียนในขั้นตอน 4</p><label>สัปดาห์ <select id="pp5Week" class="cloud-allow">'+Array.from({length:20},(_,i)=>'<option '+(week===i+1?'selected':'')+'>'+(i+1)+'</option>').join('')+'</select></label><div class="table-wrap"><table class="pp5-table"><thead><tr><th>เลขที่</th><th>ชื่อ-สกุล</th>'+[1,2,3,4,5].map(n=>'<th>ช่อง '+n+'</th>').join('')+'</tr></thead><tbody>'+Array.from({length:60},(_,i)=>{if(!value('IN','C'+(i+4))&&!value('IN','D'+(i+4)))return '';return '<tr><td>'+(i+1)+'</td><td>'+esc(value('IN','D'+(i+4)))+'</td>'+[0,1,2,3,4].map(k=>{const ref=PP5Workbook.col(7+(week-1)*5+k)+(i+7);return '<td><select data-sheet="CH" data-ref="'+ref+'" '+(!editable()?'disabled':'')+' aria-label="เวลาเรียนเลขที่ '+(i+1)+' ช่อง '+(k+1)+'">'+['','/','-','ป','ล','ส','ข'].map(v=>'<option value="'+v+'" '+(value('CH',ref)===v?'selected':'')+'>'+({'':'ยังไม่บันทึก','/':'มา','-':'ไม่มีคาบ','ป':'ป่วย','ล':'ลา','ส':'สาย','ข':'ขาด'}[v])+'</option>').join('')+'</select></td>'}).join('')+'</tr>'}).join('')+'</tbody></table></div>';
  ...
  $('importRoster').onclick=()=>{const roster=students();if(!roster.length)return message('ยังไม่มีรายชื่อ','เพิ่มนักเรียนในข้อมูลห้องเรียนก่อน');if(roster.length>60)return message('จำนวนเกินแม่แบบ','แม่แบบนี้รองรับไม่เกิน 60 คน');
   ...
   const previous=new Map();for(let i=0;i<60;i++){const id=String(value('IN','C'+(i+4))).trim();if(!id)continue;if(previous.has(id))return message('เลขประจำตัวซ้ำในเล่ม','กรุณาแก้เลขประจำตัวซ้ำก่อนเชื่อมรายชื่อ');previous.set(id,{detail:Object.fromEntries(Object.entries(patches).filter(([k])=>k.startsWith('F!')&&Number(k.match(/\d+$/)?.[0])===i+7).map(([k,v])=>[k.match(/^F!([A-Z]+)/)[1],v])),scores:[...columns,'K'].map(c=>value('IN',c+(i+4))),attendance:Array.from({length:100},(_,c)=>value('CH',PP5Workbook.col(c+7)+(i+7)))})}
   ...
   for(const k of Object.keys(patches))if(k.startsWith('F!'))delete patches[k];
   for(let i=0;i<60;i++){const r=i+4,id=String(roster[i]?.id||'').trim(),old=previous.get(id);for(const [c,v] of Object.entries(old?.detail||{}))patches['F!'+c+(i+7)]=v;put('IN','C'+r,id);put('IN','D'+r,roster[i]?.name||'');[...columns,'K'].forEach((c,k)=>put('IN',c+r,old?.scores[k]??''));for(let c=7;c<=106;c++)if(book.cells.CH[PP5Workbook.col(c)+(i+7)]?.formula==null)put('CH',PP5Workbook.col(c)+(i+7),old?.attendance[c-7]??'')}
   render();
  };
 }
